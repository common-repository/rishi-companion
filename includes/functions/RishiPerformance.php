...
class RishiCompanionPerformance {
	...
	private function init() {
        ...
        add_action( 'wp_enqueue_scripts', [ $this, 'rishi_companion_disable_elementor_pro_script' ] );
        ...
        if( get_theme_mod( 'lazy_load_featured_img', 'no' ) == 'yes' ) {
    ...
    public function rishi_companion_assets_js_init() {
        ...
        if($defer_check === 'yes' || $delay_check === 'yes' || $image_dimensions === 'yes') {
            //actions + filters
    ...
    public function rishi_companion_optimize_js( $html ) {
        ...
        if($defer_check === 'yes') {
            ...
            if(empty($excluded_arr)) {
                array_push($js_exclusions, 'jquery(?:\.min)?.js');
            }
    
            //add extra user exclusions
            if(!empty($excluded_arr) && is_array($excluded_arr)) {
    ...
    public function rishi_companion_lazy_load($html){
        ...
        if(!empty($images)) {
            ...
            foreach($images as $image) {
                ...
                $sourse_set = ($image_atts['srcset'] ? $image_atts['srcset'] : '' );
	...
	*/
	public function rishi_companion_is_elementor_activated_post(){
        ...
        if( $this->rishi_companion_is_elementor_activated() ){
            return \Elementor\Plugin::$instance->db->is_built_with_elementor( get_the_ID() ) ? true : false;     
        }else{
            return false;
        }
	}
    ...
    */
    public function rishi_companion_js_async_attr( $tag ){
        ...
        // getting the WP Plugins directory URL.
        $site_plugins_url = plugins_url();
        
        $async_files = apply_filters( 'rishi_companion_js_async_files', array( 
                get_template_directory_uri() . '/js/build/main.js',		
                RISHI_COMPANION_PLUGIN_URL . '/assets/build/main.js',
                RISHI_COMPANION_PLUGIN_URL . '/assets/build/progressBar.js',
                get_site_url() . '/wp-includes/js/jquery/jquery-migrate.min.js',
                get_site_url() . '/wp-includes/js/jquery/jquery.min.js',
                get_site_url() . '/wp-includes/js/wp-embed' . $suffix . '.js',
            )
        );
        
        if( $this->is_cf7_activated() ){
            array_push($async_files, 
                $site_plugins_url . '/contact-form-7/includes/js/scripts.js'
            );
        }
        
        if( $this->is_woocommerce_activated() ){
            array_push($async_files,
                $site_plugins_url . '/woocommerce/assets/js/frontend/cart-fragments.min.js',        	
                $site_plugins_url . '/woocommerce/assets/js/frontend/woocommerce.min.js',        	
                $site_plugins_url . '/woocommerce/assets/js/frontend/add-to-cart.min.js',
                $site_plugins_url . '/woocommerce/assets/js/jquery-blockui/jquery.blockUI.min.js',
                $site_plugins_url . '/woocommerce/assets/js/js-cookie/js.cookie.min.js'
            );
        }
        ...
        if ( !empty( $excluded_list) ){  
            ...
            foreach( $async_files as $file ){
                $individual_path = explode("/", $file);
                $element = end($individual_path);
            
                foreach( $excluded_arr as $a ){ 
                    if ( $a === $element ) { 
                        $key = array_search ($file, $async_files);
                        unset($async_files[$key]); 
                    }
                }
            }
    ...
    */
    public function rishi_companion_local_gravatar() {
        require_once plugin_dir_path(__DIR__) . 'classes/class-local-gravatar.php';  
    }
